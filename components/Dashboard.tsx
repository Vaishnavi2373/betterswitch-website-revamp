        ...
        <div className={`max-w-7xl mx-auto dashboard-section-content ${isVisible ? 'visible' : ''}`}>
          <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-24 items-center">
            ...
            <div className="space-y-4">
              <h2 className="text-3xl md:text-4xl font-extrabold tracking-tighter text-gray-900 leading-tight">
                Beyond the checkout.
              </h2>
            </div>
          ...
          <div className="md:col-span-8 relative min-h-[460px] md:min-h-[520px] w-full rounded-2xl overflow-hidden group border border-gray-200 shadow-2xl bg-[#F8FAFC] flex flex-col">
            {/* View Mode Switcher Header */}
            <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center px-4 py-3 border-b border-gray-150 bg-white gap-3 shrink-0">
              <div className="flex gap-1.5 p-0.5 bg-gray-100 rounded-lg self-start">
                <button
                  onClick={() => setViewMode('dashboard')}
                  className={`px-3.5 py-1.5 rounded-md text-xs font-bold transition-all ${viewMode === 'dashboard' ? 'bg-white text-obsidian shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
                >
                  Dashboard Observation
                </button>
                <button
                  onClick={() => setViewMode('sdk')}
                  className={`px-3.5 py-1.5 rounded-md text-xs font-bold transition-all ${viewMode === 'sdk' ? 'bg-white text-obsidian shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
                >
                ...
                <div className="flex gap-1 overflow-x-auto self-start sm:self-auto py-0.5">
                  {['curl', 'node', 'python', 'go'].map((lang) => (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md text-[10px] font-mono font-bold uppercase transition-all ${activeLang === lang ? 'bg-accent/10 text-accent border border-accent/20' : 'text-gray-500 hover:text-gray-900'}`}
                    >
            ...
            <div className="flex-1 relative overflow-hidden flex flex-col">
              {viewMode === 'dashboard' ? (
                ...
                >
                  ...
                  <g transform="translate(1080, 500)">
                    ...
                    <g transform="translate(162, 220)">
                      ...
                      <text x="0" y="5" textAnchor="middle" fontFamily="'Figtree', sans-serif" fontWeight="800" fontSize="28" fill="#0F172A">$10.2M</text>
                ...
                <div className="flex-1 bg-[#111111] text-white font-mono p-5 flex flex-col justify-between overflow-y-auto min-h-[380px]">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-stretch flex-1">
                    {/* Left: Code input editor */}
                    <div className="flex flex-col bg-[#1A1A1A] border border-white/5 rounded-xl overflow-hidden min-h-[220px]">
                      <div className="px-4 py-2 bg-[#1E1E1E] border-b border-white/5 flex items-center justify-between text-[10px] text-gray-500 font-bold tracking-wider">
                      ...
                      <pre className="p-4 text-[11px] text-gray-300 leading-relaxed overflow-x-auto whitespace-pre select-all font-mono flex-1">
                        {sdkCodeSnippets[activeLang]}
                      </pre>
                      <div className="px-4 py-2.5 bg-[#1E1E1E] border-t border-white/5 flex justify-end">
                        <button
                          onClick={runCode}
                          disabled={isRunning}
                          className="focus-ring flex items-center gap-1.5 px-4 py-2 bg-accent hover:bg-accent-hover disabled:bg-gray-700 text-white text-[11px] font-bold rounded-lg transition-all shadow-sm"
                        >
                    ...
                    <div className="flex flex-col bg-[#1A1A1A] border border-white/5 rounded-xl overflow-hidden min-h-[220px]">
                      <div className="px-4 py-2 bg-[#1E1E1E] border-b border-white/5 flex items-center gap-2 text-[10px] text-gray-500 font-bold tracking-wider">
                        <Terminal size={12} />
                        <span>Response Output</span>
                      </div>
                      <div className="p-4 text-[11px] leading-relaxed overflow-y-auto font-mono flex-1 min-h-[160px]">
                        {isRunning && (
                        ...
                        {showTerminalOutput ? (
                          <pre className="text-green-400 whitespace-pre select-all">
                            {sdkResponseJSON}
                          </pre>
                        ) : (
                          !isRunning && <span className="text-gray-650 italic font-medium block">// Click "Run API Request" to see target response payload</span>
                        )}
                      </div>
                      <div className="px-4 py-2.5 bg-[#1E1E1E] border-t border-white/5 flex items-center justify-between text-[9px] font-semibold text-gray-500">
                        <span>STATUS: {showTerminalOutput ? '200 OK' : 'IDLE'}</span>
                        <span>LATENCY: {showTerminalOutput ? '142ms' : '0ms'}</span>
                      </div>
                    </div>
                  </div>
                </div>
